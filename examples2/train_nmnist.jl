...
function build_data_set_native(events,storage,cnt,input_shape,l_change_cnt,l_old)
    ...
    #::Array{Tuple{Vector{Int32}, Vector{Int32}, Vector{Float32}, Vector{Int8}, Vector{Int32}, Vector{Any}}}
    temp_container_store::Tuple{Vector{Int32}, Vector{Int32}, Vector{Float32}, Vector{Int8}, Vector{Int32}, Vector{Any}} = (xx,yy,tts,polarity,label,pop_stimulation)
    (cnt,temp_container_store,l_change_cnt,l_old)
end
...
function bds(dataset)
    training_order = 0:1:dataset.get_count()-1
    temp_container_store= []#::Array{Tuple{Vector{Int32}, Vector{Int32}, Vector{Float32}, Vector{Int8}, Vector{Int32}, Vector{Any}}} = []
    #storage = []
    input_shape = dataset.get_element_dimensions()
    cnt = 0
    l_change_cnt = 0
    l_old = 4
    @inbounds @showprogress for batch in 1:10:length(training_order)
        events = dataset.get_dataset_item(training_order[batch:batch+1])
        cnt,temp_container_store,l_change_cnt,l_old = build_data_set_native(events,temp_container_store,cnt,input_shape,l_change_cnt,l_old)
        #@save "part_mnmist_$cnt.jld" temp_container_store
    end

    @save "all_mnmist_complete.jld" temp_container_store
