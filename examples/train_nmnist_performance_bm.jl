...
function build_data_set_native(Odesa_ConvOdesa::Module,events,storage,cnt,input_shape)
    #@show(length(events))
    xx = Vector{UInt32}([])
    yy = Vector{UInt32}([])
    tts = Vector{Float32}([])
    ...
    I = LinearIndices(A)
    #@show(I)
    pop_stimulation= []#Vector{UInt32}([])#Vector{UInt32}(size(I))
    @inline for (ind_,ev) in enumerate(events)      
        cnt+=1
        #if cnt<13977584 
        if cnt!=283249 & cnt!=228845
            (x,y,ts,p,l) = ev
            #@show(CartesianIndex(x,y))
            push!(pop_stimulation,UInt32(I[CartesianIndex(x,y)]))
            ...
            #@show(x,y)
            #@show(linear(x,y))
            #cart=ind2subv(input_shape,(x,y))
            #@show(cart)
            #linear_indices = sub2indv(input_shape,cart)
            #@show(linear_indices,x,y)
        end
        #end
        #@show(cnt)
        #@show(last(pop_stimulation))
    end
    #@show(Int64(maximum(xx)))
    #@show(Int64(maximum(yy)))
    #@show(typeof(pop_stimulation))

    did_it_exec::Tuple{Vector{UInt32}, Vector{UInt32}, Vector{Float32}, Vector{Int8}, Vector{UInt32}, Vector{Any}} = (xx,yy,tts,polarity,label,pop_stimulation)
    #@show(typeof(did_it_exec))
    
    #@show(last(storage))
    #@show(typeof(last(storage)))
    #@show(cnt)
    (cnt,did_it_exec)
end
...
#if !isfile("yeshes_events.jld") 
    #@time events_cache,input_shape = pre_conv(dataset)
    #@save "yeshes_events.jld" events_cache input_shape
#else
"""
data conversion took Time: 0:30:18
"""
