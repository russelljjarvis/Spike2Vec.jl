...
function get_all_exempler_of_days()
    length_of_spike_mat = length(matread("../jesus/M4 analyzed2DaysV.mat")["dataIntersected"])
    final_mat = matread("../jesus/M4 analyzed2DaysV.mat")["dataIntersected"][1]["Transients"]["Raster"]
    for i in 2:length_of_spike_mat
        temp = matread("../jesus/M4 analyzed2DaysV.mat")["dataIntersected"][i]["Transients"]["Raster"]
        
        final_mat = hcat(final_mat,temp)
    end
    (nodes,times,whole_duration) = convert_bool_matrice_to_raster(final_mat,frame_width)
end
...
function load_datasets_calcium()
    ...
    FPS = matread("../jesus/M4 analyzed2DaysV.mat")["dataIntersected"][1]["Movie"]["FPS"]
    
    frame_width = 1.0/FPS #0.08099986230023408 #second, sample_rate =  12.3457#Hz
    #read_spike_dense0 = matread("../jesus/M4 analyzed2DaysV.mat")#["GC06_M1963_20191204_S1"]["Movie"]["FPS"]
    #@show(read_spike_dense0)
    final_mat = matread("../jesus/M4 analyzed2DaysV.mat")["dataIntersected"][1]["Transients"]["Raster"]
    (nodes,times,whole_duration) = convert_bool_matrice_to_ts(final_mat,frame_width)
    ...
    if false
        ...
        df = DataFrame(x=nodes, y=times)

        CSV.write("scatter_data_spikes_times_jared.csv",  df, writeheader=false)
    end
...
function give_global_scope(assign,div_spike_mat_no_displacement)
    tasks0 = Vector{Any}([])
    tasks1 = Vector{Any}([])
    ...
    for un in unique(assign)
        ...
        for (ind,a) in enumerate(assign)        
            if a==un
                #Nx,Tx = return_spike_item_from_matrix(div_spike_mat_no_displacement,ind)
                #append!(Nxag,Nx)
                #append!(Txag,Tx)   
                push!(tasks0,distmat[:,ind])
                push!(tasks1,div_spike_mat_no_displacement[:,ind])

            end
            if length(Tx)!=0
                Plots.scatter!(pscatter,Tx,Nx,legend = false, markercolor=un,markersize = 1.8,markerstrokewidth=0,alpha=0.8, bgcolor=:snow2, fontcolor=:blue, xlims=(minimum(Tx),maximum(Tx)))
            end
                
        end
        savefig("scatter_match_two.png")
    ...
    (tasks0,tasks1)
            ...
            #@inbounds for (ind,x) in enumerate(horizonta_assign)
#    if ind < length(assing_progressions)
#        stateTransMat[x,horizonta_assign[ind+1]]+=1
#    end 
#end
#div_spike_mat_no_displacement[horizonta_assign]
#function plotss_2(assign,div_spike_mat_no_displacement)

#@show(R,sort_idx,assign)

(_,_,_,_,_,_) = get_divisions(nodes,Vector{Float64}(times),resolution,numb_neurons,maxt,plot=true,metric="CV")
(_,_,_,_,_,_) = get_divisions(nodes,Vector{Float64}(times),resolution,numb_neurons,maxt,plot=true,metric="autocov")
(_,_,_,_,_,_) = get_divisions(nodes,Vector{Float64}(times),resolution,numb_neurons,maxt,plot=true,metric="LV")
#(_,_,_,_,_,_) = get_divisions(nodes,times,resolution,numb_neurons,maxt,plot=true,metric="kreuz")

(distmat,tlist,nlist,start_windows,end_windows,spike_distance_size) = get_divisions(nodes,Vector{Float64}(times),resolution,numb_neurons,maxt,plot=true)


#horizontal_sort_into_tasks(mat_of_distances
...
#sqr_distmat = sqr_distmat[sort_idx,:]
#Plots.heatmap(sqr_distmat)
#savefig("horizontal_sort_Distmat_sqaure.png")

#div_spike_mat_no_displacement = div_spike_mat_no_displacement[:,sort_idx]
#Plots.scatter(div_spike_mat_no_displacement[:,sort_idx])
#savefig("horizontal_sort_Distmat_sqaure.png")
...
(#tasks0,tasks1) = give_global_scope(horizonta_assign,div_spike_mat_no_displacement)
