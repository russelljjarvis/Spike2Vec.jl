...
# _proc_setup
# Convert all arrays in IFFF to CuArrays

@noinline function _proc_setup(::CuGPU, simdata::IFNF) 
    Adapt.adapt(CuArray, simdata)
end

_copyto_output!(outgrid, neuron::IFNF, proc::GPU) = copyto!(outgrid, gridview(grid))
...
for (f, op) in atomic_ops
    ...
    @eval begin
        function ($f)(d::SPKDATA{<:Any,R}, ::CuGPU, x, I...) where R
            A = parent(dest(d))
